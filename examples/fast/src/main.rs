//! `fast` — maximum-throughput variant of the `large` commute benchmark.
//!
//! Identical simulation (1 M agents, 100×100 Atlanta grid, 7-day commute) with
//! every available performance lever engaged:
//!
//! | Lever | Setting |
//! |---|---|
//! | Global allocator | mimalloc (avoids Windows heap fragmentation) |
//! | Hash map | `FxHashMap` for the route table (SipHash → multiply-xor) |
//! | Contact index | `FxHashMap` via `dt-sim/fx-hash` feature |
//! | Parallelism | Rayon intent phase via `dt-sim/parallel` feature |
//! | Thread count | Pinned to `available_parallelism()` physical cores |
//! | LTO / codegen | `--profile fast` → fat LTO + `codegen-units=1` |
//! | SIMD | `target-cpu=native` via `.cargo/config.toml` |
//! | Output I/O | Snapshots every 24 ticks (1/day), 1-in-100 agents, batched writes |
//!
//! Run with:
//!   cargo run -p fast --profile fast
...
/// Write agent snapshots once per day (1 snapshot per day × 7 days = 7 snapshots).
const OUTPUT_INTERVAL_TICKS: u64   = 24;
/// Write every 100th agent → 10 K visible agents per snapshot.
...
///
/// Uses `FxHashMap` instead of `std::collections::HashMap` so route lookups
/// during the apply phase use a multiply-xor hash rather than SipHash.
struct PrecomputedRouter {
    routes: FxHashMap<(u32, u32), Route>,
}

impl PrecomputedRouter {
    fn build(network: &RoadNetwork, homes: &[NodeId], works: &[NodeId]) -> Self {
        let router = DijkstraRouter;
        let mut routes = FxHashMap::with_capacity_and_hasher(
            homes.len() * works.len() * 2,
            Default::default(),
        );
        for &h in homes {
            for &w in works {
...
/// intervals.  Rows are batched into a `Vec` and written in a single call to
/// avoid per-row serialization overhead.
...
impl SimObserver for SampledObserver {
    ...
    fn on_snapshot(&mut self, tick: Tick, mobility: &MobilityStore, agents: &AgentStore) {
        // Collect all sampled rows first, then write in one call to avoid
        // per-row CSV serialisation overhead.
        let snapshot_count = agents.count.div_ceil(self.sample_rate);
        ...
        self.writer.write_snapshots(&rows).ok();
    }
...
fn main() -> Result<()> {
    println!("=== rust_dt  fast — 1 M agent commute (Atlanta 100x100 grid) ===");

    // Pin Rayon's global thread pool to the number of logical CPUs reported by
    // the OS.  Explicit initialization ensures the pool is ready before the
    // sim starts and avoids a first-tick latency spike.
    ...
    println!(
        "Agents: {AGENT_COUNT}  |  Days: {SIM_DAYS}  |  Seed: {SEED}  \
         |  threads: {num_threads}  |  parallel+fx-hash+lto+native"
    );
    ...
    // 8. Output observer.
    let out_dir = Path::new("output/fast");
