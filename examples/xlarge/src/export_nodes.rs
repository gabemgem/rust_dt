...
//!
//! Writes two JSON files to `output/xlarge/`:
//!   - `node_coords.json`   — `[{node_id, lat, lon}, …]`
//!   - `network_edges.json` — `[{from_node, to_node}, …]`
//!
//! Run with: `cargo run -p xlarge --bin export_nodes`
...
fn main() -> Result<()> {
    ...
    fs::create_dir_all("output/xlarge")?;
    ...
    fs::write("output/xlarge/node_coords.json", &coords_json)?;
    println!("Wrote output/xlarge/node_coords.json ({} nodes)", node_coords.len());
    ...
    fs::write("output/xlarge/network_edges.json", &edges_json)?;
    println!("Wrote output/xlarge/network_edges.json ({} edges)", edges.len());
