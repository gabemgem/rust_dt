//! `xlarge` — 4 M agent daily commute over a 10×10 Chicago metro road network.
//!
//! Agents are split across 50 residential nodes (western suburbs) and commute
//! to 50 commercial nodes (city center) on a staggered 3-way schedule.
//! Snapshots of 1-in-80 agents (50 K visible) are written every 8 ticks.
//!
//! Run with:
//!   cargo run -p xlarge --release

// Use mimalloc to avoid Windows heap fragmentation at 4 M agents.
#[global_allocator]
...
const AGENT_COUNT:           usize = 4_000_000;
const SEED:                  u64   = 42;
const SIM_DAYS:              u64   = 7;
const TICKS_PER_DAY:         u64   = 24;
/// Write agent snapshots every N ticks.
...
///
/// Pre-computing all 50×50 home↔work pairs takes ~10 ms; look-ups during
...
fn main() -> Result<()> {
    println!("=== rust_dt  xlarge — 4 M agent commute (Chicago 10×10 grid) ===");
    println!(
    ...
    // extra heap allocation, avoiding 4 M fragmented Vec<ScheduledActivity> allocs.
    ...
    // 8. Output observer.
    let out_dir = Path::new("output/xlarge");
